...
const Customers = () => {
    ...
    const fetchCustomers = async () => {
        try {
            const res = await axios.get('http://localhost:5000/api/customers' + (query ? `/search?query=${query}` : ''));
    ...
    const handleSaveCustomer = async (e) => {
        ...
        try {
            if (editingId) {
                const res = await axios.put(`http://localhost:5000/api/customers/${editingId}`, formData);
